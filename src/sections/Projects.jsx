  ...
  return (
    <section id="projects" className="py-20 bg-transparent border-t border-slate-800/50 relative z-10">
      <div className="container mx-auto px-6 md:px-12">
        ...
        >
          <h2 className="text-3xl md:text-5xl font-bold mb-4">My <span className="text-cyan-400">Projects</span></h2>
              ...
              {filteredProjects.map((project) => (
                ...
                >
                  ...
                  <div className="relative bg-slate-800 rounded-[15px] overflow-hidden h-full flex flex-col">
                    <div className="h-60 overflow-hidden relative border-b border-slate-700/50 flex-shrink-0">
                      <img 
                        src={project.image} 
                      alt={project.title} 
                      className="w-full h-full object-cover object-top transition-transform duration-[3000ms] ease-in-out group-hover:translate-y-[-20%] group-hover:scale-105 bg-slate-900"
                    />
                    <div className="absolute inset-0 bg-slate-900/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center gap-4">
                      {project.liveLink && (
                        <a href={project.liveLink} target="_blank" rel="noreferrer" className="p-3 bg-cyan-500 rounded-full text-slate-900 hover:scale-110 transition-transform"><FiExternalLink size={20} /></a>
                      )}
                      {project.codeLink && (
                        <a href={project.codeLink} target="_blank" rel="noreferrer" className="p-3 bg-slate-700 text-white rounded-full hover:scale-110 transition-transform"><FiGithub size={20} /></a>
                      )}
                    </div>
                  </div>
                  <div className="p-6">
