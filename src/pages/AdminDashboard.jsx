...
const AdminDashboard = () => {
  ...
  // Local state for instant drag-and-drop UI updates
  ...
  const { data: projects, isLoading } = useQuery({
    ...
    queryFn: async () => {
      ...
      // Sort projects by their order field ascending
      const sortedData = Array.isArray(res.data) ? res.data.sort((a, b) => (a.order || 0) - (b.order || 0)) : [];
      return sortedData;
    },
    enabled: !!token
  });
  ...
  // Mutation to save the new order to the backend
  const reorderMutation = useMutation({
    mutationFn: async (sortedProjects) => {
      return await axiosInstance.put('/projects/reorder', { sortedProjects });
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['admin_projects'] });
      toast.success('Projects reorganized successfully!');
    },
    onError: () => {
  ...
  const handleSubmit = (e) => {
    ...
    if (editingProject) {
      ...
      // Append new project to the end of the list
      createMutation.mutate({ ...apiData, order: localProjects.length });
    }
  };
  ...
  // Main function to handle sorting logic
  const handleOnDragEnd = (result) => {
    ...
    // 2. Trigger API mutation to save order
              ...
              <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
                {isLoading ? (
                  ...
                  <div className="w-full">
                    ...
                    <div className="grid grid-cols-12 gap-4 p-4 bg-slate-800 border-b border-slate-700 font-medium text-slate-300 text-sm">
                      <div className="col-span-6 pl-8">Project</div>
                              ...
                              <Draggable key={proj._id} draggableId={proj._id} index={index}>
                                {(provided, snapshot) => (
                                  <div
                                    ...
                                    className={`grid grid-cols-12 gap-4 p-4 items-center border-b border-slate-800/50 transition-colors select-none ${
                                      snapshot.isDragging ? 'bg-slate-800 border-y border-cyan-500/50 shadow-2xl' : 'hover:bg-slate-800/20'
                                    }`}
                                  >
                                    ...
                                    <div className="col-span-6 flex items-center gap-3">
                                      ...
                                      <img 
                                        src={proj.image || 'https://via.placeholder.com/50'} 
                                        alt={proj.title} 
                                        className="w-12 h-12 rounded object-cover border border-slate-700 pointer-events-none" 
                                      />
                                      <div className="min-w-0">
                                        <p className="font-semibold text-white truncate">{proj.title}</p>
                                        <p className="text-xs text-slate-500 truncate max-w-[200px] md:max-w-[300px]">{proj.description}</p>
                                      </div>
                                    </div>

                                    {/* Category */}
                                    <div className="col-span-2">
                                      <span className="px-2 py-1 bg-slate-800 border border-slate-700/50 rounded text-xs text-cyan-400">
                                        {proj.category}
                                      </span>
                                    </div>

                                    {/* Links */}
                                    <div className="col-span-2 space-x-2 text-xs">
                                      {proj.liveLink && <a href={proj.liveLink} target="_blank" rel="noreferrer" className="text-blue-400 hover:underline">Live</a>}
                                      {proj.codeLink && <a href={proj.codeLink} target="_blank" rel="noreferrer" className="text-purple-400 hover:underline">Code</a>}
                                    </div>

                                    {/* Actions */}
                                    <div className="col-span-2 text-right">
                                      <div className="flex justify-end gap-2">
                                        <button onClick={() => openModal(proj)} className="p-2 bg-slate-800 border border-slate-700/50 text-slate-300 rounded hover:text-cyan-400 transition-colors"><FiEdit /></button>
                                        <button onClick={() => handleDelete(proj._id)} className="p-2 bg-slate-800 border border-slate-700/50 text-slate-300 rounded hover:text-red-400 transition-colors"><FiTrash2 /></button>
                                      </div>
                                    </div>
                                  </div>
                                )}
