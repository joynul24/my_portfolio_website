...
// About Section Settings Component
const AboutSettings = () => {
  const queryClient = useQueryClient();
  ...
  return (
    <div className="bg-slate-900 border border-slate-800 rounded-2xl p-8">
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        ...
        <button type="submit" disabled={updateMutation.isPending} className="mt-4 px-6 py-2 bg-cyan-500 text-slate-900 rounded-lg font-bold hover:bg-cyan-400 disabled:opacity-70">
          Save Changes
        </button>
      </form>
    </div>
    ...
    <div>
      <div className="bg-slate-900 border border-slate-800 rounded-2xl p-6 mb-8">
        ...
        <form onSubmit={handleAdd} className="flex flex-wrap gap-4 items-end">
          ...
          <div className="w-32">
            ...
            <input type="number" min="0" max="100" required value={formData.level} onChange={(e) => setFormData({...formData, level: parseInt(e.target.value) || 0})} className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white outline-none focus:border-cyan-500" />
          </div>
          <div className="w-48">
            <label className="block text-sm text-slate-400 mb-1">Category</label>
            <select value={formData.category} onChange={(e) => setFormData({...formData, category: e.target.value})} className="w-full px-3 py-2 bg-slate-950 border border-slate-800 rounded-lg text-white outline-none focus:border-cyan-500 cursor-pointer">
              <option>Frontend</option>
              <option>Backend</option>
              <option>Tools</option>
            </select>
          </div>
          <button type="submit" disabled={createMutation.isPending} className="px-5 py-2 bg-cyan-500 text-slate-900 rounded-lg font-bold hover:bg-cyan-400 disabled:opacity-70 h-[42px]">
            Add Skill
          </button>
        </form>
      </div>
      ...
      <div className="bg-slate-900 border border-slate-800 rounded-2xl overflow-hidden">
        {isLoading ? (
          ...
          <p className="p-6 text-slate-400 text-center">No skills found. Add one!</p>
        ) : (
          <table className="w-full text-left">
            <thead className="bg-slate-800 border-b border-slate-700">
              <tr>
            ...
            <tbody>
              {skills?.map((skill) => (
                <tr key={skill._id} className="border-b border-slate-800/50 hover:bg-slate-800/20">
                  ...
                  <td className="p-4 text-right">
                    <button onClick={() => handleDeleteSkill(skill._id)} className="p-2 bg-slate-800 text-slate-300 rounded hover:text-red-400 transition-colors"><FiTrash2 /></button>
                  </td>
                </tr>
              ))}
            </tbody>
